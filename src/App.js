...
function Banner({ user, tab, setTab, phase, onLogout }) {
  ...
  const NAV_U = [
    ...
    { id: 'games',       icon: '🎮',  img: null,          label: 'Spill' },
  ];
  const NAV_A = [
...
// ── Games Page ────────────────────────────────────────────────────────
function GamesPage({ me }) {
  const [activeGame, setActiveGame] = useState('penalty');
  return (
    <div>
      <div style={{ display:'flex', gap:8, marginBottom:16 }}>
        {[{id:'penalty',label:'⚽ Straffespark'},{id:'keeper',label:'🧤 Keeperspill'}].map(g=>(
          <button key={g.id} onClick={()=>setActiveGame(g.id)}
            style={{ flex:1, padding:'10px', borderRadius:10, border:'1px solid',
              borderColor: activeGame===g.id ? 'rgba(255,215,0,.4)' : 'rgba(255,255,255,.1)',
              background: activeGame===g.id ? 'rgba(255,215,0,.1)' : 'rgba(255,255,255,.04)',
              color: activeGame===g.id ? '#FFD700' : 'rgba(255,255,255,.6)',
      ...
      {activeGame==='penalty' && <PenaltyGame me={me} />}
      {activeGame==='keeper'  && <KeeperGame  me={me} />}
    </div>
  );
}

// ── Penalty Game ──────────────────────────────────────────────────────
function PenaltyGame({ me }) {
  const cRef = useRef(null);
  const [score,  setScore]  = useState(0);
  const [shots,  setShots]  = useState(5);
  const [msg,    setMsg]    = useState('Klikk i målet for å skyte!');
  const [done,   setDone]   = useState(false);
  const [hs,     setHs]     = useState({ score:0, name:'' });
  const phaseRef = useRef('idle');
  const ballRef  = useRef({ x:0.5, y:0.9 });
  const keeperRef= useRef(0.5);
  const scoreRef = useRef(0);
  const shotsRef = useRef(5);

  useEffect(() => {
    getDoc(doc(db,'gameHS','penalty')).then(s=>{
      if(s.exists()) setHs(s.data());
    });
  }, []);
  ...
  function draw() {
    ...
    for(let i=0;i<W;i+=50){ ctx.fillStyle=i%100===0?'#1d641d':'#185418'; ctx.fillRect(i,0,50,H); }
    // Goal
    const gw=W*0.72,gh=H*0.44,gx=(W-gw)/2,gy=10;
    ctx.fillStyle='rgba(0,0,0,.25)'; ctx.fillRect(gx,gy,gw,gh);
    ctx.strokeStyle='#fff'; ctx.lineWidth=4; ctx.strokeRect(gx,gy,gw,gh);
    ctx.strokeStyle='rgba(255,255,255,.15)'; ctx.lineWidth=1;
    for(let col=1;col<3;col++){ ctx.beginPath(); ctx.moveTo(gx+col*gw/3,gy); ctx.lineTo(gx+col*gw/3,gy+gh); ctx.stroke(); }
    ctx.beginPath(); ctx.moveTo(gx,gy+gh/2); ctx.lineTo(gx+gw,gy+gh/2); ctx.stroke();
    // Keeper
    const kx=gx+keeperRef.current*gw;
    ctx.fillStyle='#00e5ff'; ctx.beginPath(); ctx.arc(kx,gy+18,16,0,Math.PI*2); ctx.fill();
    ctx.fillStyle='#0088cc'; ctx.fillRect(kx-18,gy+34,36,44);
    ctx.fillStyle='rgba(255,255,255,.3)'; ctx.fillRect(kx-18,gy+34,36,12);
    // Penalty spot
    ctx.fillStyle='#fff'; ctx.beginPath(); ctx.arc(W/2,H-28,5,0,Math.PI*2); ctx.fill();
    // Ball
    const bx=ballRef.current.x*W, by=ballRef.current.y*H;
    ctx.fillStyle='#fff'; ctx.beginPath(); ctx.arc(bx,by,13,0,Math.PI*2); ctx.fill();
    ctx.strokeStyle='#222'; ctx.lineWidth=1.5; ctx.stroke();
    // Pentagon pattern on ball
    ctx.fillStyle='#333';
    for(let i=0;i<5;i++){
      const a=i*Math.PI*2/5-Math.PI/2;
      ctx.beginPath(); ctx.arc(bx+Math.cos(a)*5,by+Math.sin(a)*5,3,0,Math.PI*2); ctx.fill();
    }
  }

  async function shoot(e) {
    if(done||phaseRef.current!=='idle'||shotsRef.current<=0) return;
    const wrap=cRef.current.parentElement;
    ...
    const inGoal=rx>=gx&&rx<=gx+gw&&ry<=gh+10/H;
    const kDist=Math.abs(rx-(gx+keeperRef.current*gw));
    const saved=inGoal&&kDist<0.1&&ry<0.4;
    let result = !inGoal ? 'miss' : saved ? 'saved' : 'goal';
    let steps=0;
    const animate=setInterval(()=>{
      steps++;
      const t=Math.min(steps/18,1);
      ballRef.current={x:0.5+(rx-0.5)*t, y:0.9-(0.9-ry)*t};
      draw();
      if(steps>=18){
        clearInterval(animate);
        phaseRef.current='idle';
        ballRef.current={x:0.5,y:0.9};
        shotsRef.current--;
        if(result==='goal'){ scoreRef.current++; setScore(scoreRef.current); }
        setShots(shotsRef.current);
        setMsg(result==='goal'?'⚽ MÅL!':result==='saved'?'🧤 Keeper redder!':'😬 Utenfor!');
        if(shotsRef.current<=0){
          setDone(true);
          getDoc(doc(db,'gameHS','penalty')).then(async s=>{
            const cur=s.exists()?s.data():{score:0,name:''};
            if(scoreRef.current>cur.score){
              const newHs={score:scoreRef.current,name:me.displayName};
              await setDoc(doc(db,'gameHS','penalty'),newHs);
              setHs(newHs);
              setMsg('🏆 NY REKORD! '+scoreRef.current+'/5 mål!');
            } else {
              setMsg('Ferdig! '+scoreRef.current+'/5 mål. Rekord: '+cur.score+' ('+cur.name+')');
            }
          });
        }
      }
  ...
  function reset(){
    scoreRef.current=0; shotsRef.current=5;
    ballRef.current={x:0.5,y:0.9}; keeperRef.current=0.5;
    setScore(0); setShots(5); setDone(false);
    setMsg('Klikk i målet for å skyte!');
    draw();
  }

  return (
    <div style={C.card}>
      <div style={C.cardHeader}><span style={C.cardTitle}><span style={C.cardTitleDot}/>Straffespark</span></div>
      <div style={C.cardBody}>
        <div style={{display:'flex',gap:16,marginBottom:12,alignItems:'center'}}>
          <div style={C.scoreBox}><div style={{...C.scoreNum,fontSize:32,color:'#FFD700'}}>{score}</div><div style={C.scoreL}>Mål</div></div>
          <div style={C.scoreBox}><div style={{...C.scoreNum,fontSize:32,color:'#e8edf8'}}>{shots}</div><div style={C.scoreL}>Igjen</div></div>
          <div style={{...C.scoreBox,flex:1}}>
            <div style={{...C.scoreNum,fontSize:20,color:'#4ade80'}}>{hs.score}</div>
            <div style={C.scoreL}>Rekord</div>
            {hs.name&&<div style={{fontSize:10,color:'rgba(255,255,255,.4)',marginTop:2}}>{hs.name}</div>}
          </div>
        </div>
        <div style={{position:'relative',borderRadius:12,overflow:'hidden',cursor:'crosshair'}} onClick={shoot}>
          <canvas ref={cRef} width={600} height={220} style={{width:'100%',height:220,display:'block'}}/>
        </div>
        <div style={{textAlign:'center',padding:'10px 0',fontSize:15,fontWeight:700,color:'#e8edf8',minHeight:36}}>{msg}</div>
        <button style={{...C.btnGold,width:'100%'}} onClick={reset}>Ny runde</button>
      </div>
    </div>
  );
}
...
// ── Keeper Game ───────────────────────────────────────────────────────
function KeeperGame({ me }) {
  const cRef    = useRef(null);
  const stateRef= useRef({running:false,score:0,miss:0,keeper:{x:0.5,y:0.5},ball:null,ballV:null,animId:null,launchId:null});
  const [score,  setScore]  = useState(0);
  const [miss,   setMiss]   = useState(0);
  const [msg,    setMsg]    = useState('Trykk Start – beveg musa for å redde!');
  const [running,setRunning]= useState(false);
  const [hs,     setHs]     = useState({score:0,name:''});

  useEffect(()=>{
    getDoc(doc(db,'gameHS','keeper')).then(s=>{ if(s.exists()) setHs(s.data()); });
    draw();
  },[]);
  ...
  function draw(){
    ...
    ctx.strokeStyle='#fff'; ctx.lineWidth=4; ctx.strokeRect(gx,gy,gw,gh);
    // Keeper
    const kx=gx+st.keeper.x*gw, ky=gy+st.keeper.y*gh*0.75+16;
    ctx.fillStyle='#FFD700'; ctx.beginPath(); ctx.arc(kx,ky-12,16,0,Math.PI*2); ctx.fill();
    ctx.fillStyle='#b8960a'; ctx.fillRect(kx-18,ky,36,44);
    ctx.fillStyle='rgba(255,255,255,.25)'; ctx.fillRect(kx-18,ky,36,12);
    // Ball
    if(st.ball){
      ctx.fillStyle='#fff'; ctx.beginPath(); ctx.arc(st.ball.x,st.ball.y,12,0,Math.PI*2); ctx.fill();
      ctx.strokeStyle='#222'; ctx.lineWidth=1.5; ctx.stroke();
    }
  }
  ...
  function launch(){
    ...
    const gw=W*0.72,gx=(W-gw)/2,gh=H*0.44,gy=10;
    const tx=gx+Math.random()*gw*0.9+gw*0.05;
    const ty=gy+5+Math.random()*gh*0.8;
    const sx=W/2, sy=H+15;
    const dx=tx-sx, dy=ty-sy;
    const len=Math.sqrt(dx*dx+dy*dy);
    const spd=10+Math.random()*8;
    st.ball={x:sx,y:sy};
    st.ballV={x:dx/len*spd,y:dy/len*spd};
    animate();
  }

  function animate(){
    const c=cRef.current; if(!c) return;
    const st=stateRef.current; if(!st.running||!st.ball) return;
    const W=c.width,H=c.height;
    const gw=W*0.72,gx=(W-gw)/2,gh=H*0.44,gy=10;
    st.ball.x+=st.ballV.x; st.ball.y+=st.ballV.y;
    // Keeper collision
    const kx=gx+st.keeper.x*gw, ky=gy+st.keeper.y*gh*0.75+16;
    const dist=Math.sqrt((st.ball.x-kx)**2+(st.ball.y-ky)**2);
    if(dist<36){
      st.score++; setScore(st.score);
      setMsg('🧤 Reddet!');
      st.ball=null;
      st.launchId=setTimeout(launch,700);
      draw(); return;
    }
    // Goal
    if(st.ball.y<gy+gh&&st.ball.y>gy&&st.ball.x>gx&&st.ball.x<gx+gw&&st.ballV.y<0){
      st.miss++; setMiss(st.miss);
      setMsg('😱 Innmål!');
      st.ball=null;
      if(st.miss>=3){ stop(); return; }
      st.launchId=setTimeout(launch,700);
      draw(); return;
    }
    if(st.ball.y<-40||st.ball.x<-40||st.ball.x>W+40){ st.ball=null; st.launchId=setTimeout(launch,400); draw(); return; }
    draw();
    st.animId=requestAnimationFrame(animate);
  }

  function moveKeeper(e){
    const wrap=cRef.current?.parentElement; if(!wrap) return;
    const rect=wrap.getBoundingClientRect();
    const st=stateRef.current;
    st.keeper.x=Math.max(0,Math.min(1,(e.clientX-rect.left)/rect.width));
    st.keeper.y=Math.max(0,Math.min(1,(e.clientY-rect.top)/rect.height));
    if(!st.running) draw();
  }

  function start(){
    const st=stateRef.current;
    st.running=true; st.score=0; st.miss=0; st.ball=null;
    setScore(0); setMiss(0); setRunning(true);
    setMsg('Beveg musa for å redde skuddene!');
    setTimeout(launch,1000);
  }

  async function stop(){
    const st=stateRef.current;
    st.running=false; st.ball=null;
    if(st.animId) cancelAnimationFrame(st.animId);
    if(st.launchId) clearTimeout(st.launchId);
    setRunning(false);
    draw();
    const cur=await getDoc(doc(db,'gameHS','keeper'));
    const curHS=cur.exists()?cur.data():{score:0,name:''};
    if(st.score>curHS.score){
      const newHs={score:st.score,name:me.displayName};
      await setDoc(doc(db,'gameHS','keeper'),newHs);
      setHs(newHs);
      setMsg('🏆 NY REKORD! '+st.score+' redninger!');
    } else {
      setMsg('Slutt! '+st.score+' redninger. Rekord: '+curHS.score+' ('+curHS.name+')');
    }
  }

  return (
    <div style={C.card}>
      <div style={C.cardHeader}><span style={C.cardTitle}><span style={C.cardTitleDot}/>Keeperspill</span></div>
      <div style={C.cardBody}>
        <div style={{display:'flex',gap:12,marginBottom:12}}>
          <div style={C.scoreBox}><div style={{...C.scoreNum,fontSize:32,color:'#4ade80'}}>{score}</div><div style={C.scoreL}>Reddet</div></div>
          <div style={C.scoreBox}><div style={{...C.scoreNum,fontSize:32,color:'#f87171'}}>{miss}</div><div style={C.scoreL}>Innsluppet</div></div>
          <div style={{...C.scoreBox,flex:1}}>
            <div style={{...C.scoreNum,fontSize:20,color:'#FFD700'}}>{hs.score}</div>
            <div style={C.scoreL}>Rekord</div>
            {hs.name&&<div style={{fontSize:10,color:'rgba(255,255,255,.4)',marginTop:2}}>{hs.name}</div>}
          </div>
        </div>
        <div style={{position:'relative',borderRadius:12,overflow:'hidden',cursor:'none'}}
          onMouseMove={moveKeeper}>
          <canvas ref={cRef} width={600} height={220} style={{width:'100%',height:220,display:'block'}}/>
        </div>
        <div style={{textAlign:'center',padding:'10px 0',fontSize:14,fontWeight:700,color:'#e8edf8',minHeight:36}}>{msg}</div>
        <div style={{display:'flex',gap:8}}>
          <button style={{...C.btnGold,flex:1}} onClick={start} disabled={running}>Start</button>
          <button style={{...C.btnSecondary,flex:1}} onClick={stop} disabled={!running}>Stopp</button>
        </div>
        <p style={{fontSize:11,color:'rgba(255,255,255,.35)',marginTop:8,textAlign:'center'}}>3 innslupne mål = game over</p>
      </div>
    </div>
  );
}
  ...
  return (
    <div style={C.app}>
      ...
      <div style={C.main}>
        ...
        {tab === 'games'       && !user.isAdmin && <GamesPage me={user} />}
